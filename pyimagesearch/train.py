...
from utils import set_seed, to_device, save_model, log_model_preds, get_class_name_in_snake_case as snake_case
import params
...
tfms = {"train": [T.Resize(default_cfg.img_size), T.ToTensor(), T.RandomHorizontalFlip()],
...
def train(cfg):
    with wandb.init(project=cfg.PROJECT_NAME, entity=cfg.ENTITY, job_type="training", config=cfg):
        set_seed(cfg.seed)

        cfg = wandb.config
        df, processed_dataset_dir = prepare_data(cfg.PROCESSED_DATA_AT)

        train_ds = ImageDataset(df[~df.valid], 
                                processed_dataset_dir, 
                                image_column=cfg.image_column,
                                target_column=cfg.target_column,
                                transform=tfms["train"])
        
        valid_ds = ImageDataset(df[df.valid], 
                                processed_dataset_dir, 
                                image_column=cfg.image_column,
                                target_column=cfg.target_column,
